...
hi=5
hf=5
start_hour='2025-01-01T%s'%(hi)
end_hour  ='2025-01-01T%s'%(hf)
...
dates=['TS_10cm','TS_20cm','TS_40cm','US_10cm','US_20cm','US_30cm','US_40cm','US_60cm','US_100cm','Flx_S_5cm','Time','date']
...
mean_sh   = down.hour_set_group(shca_sh)
...
##
soil_shca =   mean_sh.loc[diurnalshca]
soil_iop1 = mean_iop1.loc[diurnaliop1]
soil_iop2 = mean_iop2.loc[diurnaliop2]
print(soil_shca)
print('xx')
print(soil_iop1)
print('xx')
...
file1= open('%s/%s'%(path,label1)  ,'w+')
file2= open('%s/%s'%(path,label2)  ,'w+')
file3= open('%s/%s'%(path,label3)  ,'w+')
